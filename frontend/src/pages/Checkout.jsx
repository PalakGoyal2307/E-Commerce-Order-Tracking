...
function Checkout() {
  ...
  useEffect(() => {
    if (user?._id) {
      axios.get(`http://localhost:5000/api/address/${user._id}`).then((res) => setAddresses(res.data));
    }
  }, [user]);
  ...
  const handleAddAddress = async () => {
    ...
    const { data } = await axios.post("http://localhost:5000/api/address/add", {
      ...newAddress,
      userId: user._id
    });
  ...
  const handleUpdateAddress = async () => {
    const { data } = await axios.put(`http://localhost:5000/api/address/update/${editId}`, newAddress);
